...
class UNetSubModel():
    def __init__(self, input_layer, output_channels, kernel_sizes):
        ...
        self.deconv2 = Concatenate()([self.deconv2, self.pool4])
        self.deconv2 = Conv2D(filters = 256, kernel_size = (3, 3), padding = 'same')(self.deconv2)
        self.deconv2 = LeakyReLU(alpha = 0.1)(self.deconv2)
        ...
        self.deconv3 = Concatenate()([self.deconv3, self.pool3])
        self.deconv3 = Conv2D(filters = 128, kernel_size = (3, 3), padding = 'same')(self.deconv3)
        self.deconv3 = LeakyReLU(alpha = 0.1)(self.deconv3)
        ...
        self.deconv4 = Concatenate()([self.deconv4, self.pool2])
        ...
        self.deconv5 = Concatenate()([self.deconv5, self.pool1])
        self.deconv5 = Conv2D(filters = 32, kernel_size = (7, 7), padding = 'same')(self.deconv5)
        self.deconv5 = LeakyReLU(alpha = 0.1)(self.deconv5)

        self.output_layer = Conv2D(filters = output_channels, kernel_size = (1, 1))(self.deconv5)
        self.output_layer = LeakyReLU(alpha = 0.1)

    def get_output(self, return_multiple):
            return self.pool6, self.output_layer
    
class Model():
    def __init__(self, frame_1, frame_2, timestamp):
        self.input_1 = Concatenate()([frame_1, frame_2])
        self.input_layer_1 = Input()(self.input_1)
        self.unet_encoding_output, self.unet_final_output = UNetSubModel(self.input_layer_1, output_channels = 4, kernel_sizes = (7, 5)).get_output()
        ...
        self.output_1_1 = -1 * (1 - timestamp) * timestamp * self.output_1 + timestamp * timestamp * self.output_2
        self.output_2_1 = (1 - timestamp) * (1 - timestamp) * self.output_1 - timestamp * (1 - timestamp) * self.output_2

        self.input_2 = Concatenate()([frame_1, frame_2, self.__backward_warping(frame_2, self.output_2_1), self.__backward_warping(frame_1, self.output_1_1), self.output_1_1, self.output_2_1])
        self.input_layer_2 = Input()(self.input_2)

        _, self.output_2 = UNetSubModel(self.input_layer_2, output_channels = 5, kernel_sizes = (3, 3)).get_output()
        ...
        self.visibility_1 = K.tile(self.visibility_1, (1, 1, 1, 3))

        self.visibility_2 = 1 - self.visibility_1

        self.output_1_2 = self.output_1_1 + self.output_1
        self.output_2_2 = self.output_2_1 + self.output_2

        self.output = ((1 - timestamp) * self.visibility_1) * self.__backward_warping(frame_1, self.output_1_2) + (timestamp * self.visibility_2) * self.__backward_warping(frame_2, self.output_2_2)
        self.output = (self.output) / ((1 - timestamp) * self.visibility_1 + timestamp * self.visibility_2 + 1e-12)

        self.model = Model(inputs = [timestamp, self.input_layer_1, self.input_layer_2], output = self.output)

    def __backward_warping(self, frame, output, crop_size = None, perform_resize = False, normalize = False, output_type = 'CONSTANT'):
        shape = frame.shape
        ...
        output_x, output_y = tf.split(output, 2, axis = 3)
        n, h, w = self.__get_grid(frame.shape[0], H, W, h, w)

        if(normalize):
            output_y = output_y * tf.cast(H, dtype = tf.float32)
            output_y /= 2
            output_x = output_x * tf.cast(W, dtype = tf.float32)
            output_x /= 2
    ...
    def __get_grid(self, channels, H, W, h, w):
